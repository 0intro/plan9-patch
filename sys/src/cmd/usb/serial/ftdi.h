...
enum {
	...
	FTGETE2READ	= 0x90,		/* Read an address from 128-byte I2C EEPROM */
	FTSETE2WRITE,			/* Write to address on the 128-byte I2C EEPROM */
	FTSETE2ERASE,			/* Erase address on the 128-byte I2C EEPROM */
};
 ...
 */
enum {
	BMSERIAL = 0, 			/* reset, turn off bitbang mode */

	BIGBMNORMAL = 1,			/* normal big bang mode */		
	BIGBMSPI = 2,				/* spi big bang mode */

	BMABM = 1<<8, 			/* async mode */
	BMMPSSE = 2<<8,
	BMSYNCBB = 4<<8,		/* sync bitbang -- 2232x and R-type */
 ...
 * wValue: Latency (milliseconds 1-255ms)
 * wIndex: Port
 */
enum {
	FTLATENCYDEFAULT = 2,
