...
slwrite(Serialport *p, int req, void *buf, int len)
{
	...
	ser = p->s;
	res = usbcmd(ser->dev, Rh2d | Rvendor | Riface, req, 0, p->interfc, buf, len);
	return res;
}
...
slput(Serialport *p, uint op, uint val)
{
	...
	ser = p->s;
	res = usbcmd(ser->dev, Rh2d | Rvendor | Riface, op, val, p->interfc, nil, 0);
	return res;
}
...
slread(Serialport *p, int req, void *buf, int len)
{
	...
	ser = p->s;
	res = usbcmd(ser->dev, Rd2h | Rvendor | Riface, req, 0, p->interfc, buf, len);
	return res;
}
