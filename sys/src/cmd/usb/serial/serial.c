...
{
	int c, i, n, nf, nop, nw, par, drain, set, lines, res;
	char *f[16];
	uchar x;
...
altwrite(Serial *ser, uchar *buf, long count)
{
	...
	do{	
		if(ser->wait4write != nil)
			nw = ser->wait4write(ser, buf, count);	/* unlocked inside later */
		else{
			dfd = ser->epout->dfd;
			qunlock(ser);
...
dwrite(Usbfs *fs, Fid *fid, void *buf, long count, vlong)
{
	...
	switch(path){
	case Qdata:
		nw = altwrite(ser, (uchar *)buf, count);
		count = nw;
		break;
	case Qctl:
...
serialmain(Dev *dev, int argc, char* argv[])
{
	...
	ser->w4data = chancreate(sizeof(ulong), 0);	/* BUG could this go wrong?,channel leaks? */
