...
Jconv(Fmt *fp)
{
	...
	case FT_DCREATE0:
	case FT_DCREATE1:
		rval = fmtprint(fp, "create f %ld p %ld t %lud m %ulo %s",
			j->fnum, j->parent, j->mtime, j->mode, j->name);
		break;
	case FT_chmod:
	case FT_CHMOD0:
	case FT_CHMOD1:
		rval = fmtprint(fp, "chmod f %ld m %ulo #%ld",
	...
	case FT_REMOVE:
		...
		break;
	case FT_WRITE:
		rval = fmtprint(fp, "write f %ld z %ld o %ld t %uld",
			j->fnum, j->size, j->offset, j->mtime);
		break;
	case FT_AWRITE:
		rval = fmtprint(fp, "awrite f %ld z %ld o %ld",
			j->fnum, j->size, j->offset);
		break;
	case FT_trunc:
	case FT_TRUNC0:
	case FT_TRUNC1:
		rval = fmtprint(fp, "trunc f %ld o %ld p %ld t %ld m %ulo %s",
			j->fnum, j->tnum, j->parent, j->mtime, j->mode, j->name);
		break;
	case FT_SUMMARY:
		rval = fmtprint(fp, "summary %ld",
			j->seq);
		break;
	case FT_SUMBEG:
		rval = fmtprint(fp, "sumbeg %ld",
			j->seq);
		break;
	case FT_SUMEND:
		rval = fmtprint(fp, "end");
		break;
	default:
		rval = fmtprint(fp, "?type %d", j->type);
	}
	return rval;
}
