...
rdpart(Edit *edit, uvlong lba, uvlong xbase)
{
	...
	diskread(edit->disk, &table, Tablesize, mbroffset+lba, Toffset);
...
wrextend(Edit *edit, int i, vlong xbase, vlong startlba, vlong *endlba)
{
	...
	Finish:
		if(startlba < *endlba){
			disk = edit->disk;
			diskread(disk, &table, Tablesize, mbroffset+startlba, Toffset);
			tp = table.entry;
			ep = tp+NTentry;
			for(; tp<ep; tp++)
			...
			if(diskwrite(edit->disk, &table, Tablesize, mbroffset+startlba, Toffset) < 0)
				recover(edit);
		}
		return i;
	}
...
{	
	...
	diskread(disk, &table, Tablesize, mbroffset, Toffset);
	...
	if(diskwrite(disk, &table, Tablesize, mbroffset, Toffset) < 0)
		recover(edit);
