...
i82563ifstat(Ctlr *c)
{
	...
	for(i = 0; i < Nstatistics; i++){
		...
		case Totl:
			ruvl = r;
			ruvl += ((uvlong)Get(c, Statistics+(i+1)*4))<<32;
			tuvl = ruvl;
			tuvl += c->statistics[i];
...
i82563txinit(Ctlr* c)
{
	...
	Set(c, Tctl, (0x0F<<CtSHIFT)|Psp|(66<<ColdSHIFT));
	Set(c, Tipg, (6<<20)|(8<<10)|8);
...
//	Set(c, Rctl, Lpe|Dpf|Bsize16384|Bam|RdtmsHALF|Bsex|Secrc);
	Set(c, Rctl, Lpe|Dpf|Bsize8192|Bam|RdtmsHALF|Bsex|Secrc);
	...
	i82563replenish(c);
	Set(c, Radv, 64);
//	Set(c, Rxdctl, (8<<WthreshSHIFT)|(8<<HthreshSHIFT)|4);
//	Set(c, Rxdctl, (0<<WthreshSHIFT)|(0<<HthreshSHIFT)|0);
	Set(c, Rxdctl, (1<<16)|(1<<24));

	/*
	 * Enable checksum offload.
	 */
//#define ETHERHDRSIZE 14
//	Set(c, Rxcsum, Tuofl|Ipofl|(ETHERHDRSIZE<<PcssSHIFT));
}
...
i82563attach(Ether *e)
{
	...
	for(c->nrb = 0; c->nrb < Nrb; c->nrb++){
		m = mballoc(Rbsz+BY2PG, 0, Mbeth82563);
		m->free = i82563rbfree;
		mbfree(m);
	}
...
{
	Set(c, Eerd, ee_start | adr << 2);
	while ((Get(c, Eerd) & ee_done) == 0)
		;
...
reset(Ctlr *c)
{
	...
	Set(c, Fcah, 0x00000100);
	Set(c, Fct, 0x00008808);
	Set(c, Fcttv, 0x00000100);
	Set(c, Fcrtl, c->fcrtl);
	Set(c, Fcrth, c->fcrth);
	return 0;
}
