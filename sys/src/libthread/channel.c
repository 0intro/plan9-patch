...
chanclose(Channel *c)
{
	 ...
	 * Hopefully this is O(2n) and not O(n*n)
	 */
	do{
		some = 0;
		for(i=0; i<c->nentry; i++){
			...
			*a->tag = c;
			unlock(&chanlock);
			_procsplx(s);
			while(_threadrendezvous(a->tag, Closed) == Inted)
				;
			s = _procsplhi();
			lock(&chanlock);
			some++;
...
runop(int op, Channel *c, void *v, int nb)
{
	...
	case 1:	/* nonblocking, didn't accomplish anything */
		...
		return 0;
	case 0:
		/* Ok. but return -1 if the op is done because of a
		 * close.
		 */
		if(a[0].err != nil)
			return -1;
...
altexec(Alt *a, int spl)
{
	...
	if(b != nil){
		...
		*b->tag = c;	/* commits us to rendezvous */
		...
		while(_threadrendezvous(b->tag, 0) == Inted)
			;
		return 1;
	}
