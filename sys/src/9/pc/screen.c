...
hwdraw(Memdrawparam *par)
{
	...
	if(scr->gscreen == nil || scr->gscreendata == nil)
		return 0;
	if(scr->cur == &swcursor){
		if(dst->data->bdata == scr->gscreendata->bdata)
			swcursoravoid(par->r);
		if(src && src->data->bdata == scr->gscreendata->bdata)
			swcursoravoid(par->sr);
		if(mask && mask->data->bdata == scr->gscreendata->bdata)
			swcursoravoid(par->mr);
	}
	if(hwaccel == 0)
		return 0;
	if(dst->data->bdata != scr->gscreendata->bdata || src == nil || mask == nil)
		return 0;
