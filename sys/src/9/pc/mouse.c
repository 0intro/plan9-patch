	...
	if(++nb == packetsize){
		...
		if(intellimouse && packetsize==4){
			if((msg[3]&0xc8) == 0x08){
				 ...
				 * furthermore, intellimice with 3buttons + scroll give a two's complement
				 * number in the lower 4 bytes (byte 4 is sign extension) that describes
				 * the amount the scroll wheel has moved during the last sample. here we use only 
				 * the sign to decide whether the wheel is moving up or down and generate 
				 * a single button 4 or 5 click accordingly.
				 */
				if((msg[3] >> 3) & 1) 
					buttons |= 1<<3;
				else if(msg[3] & 0x7) 
