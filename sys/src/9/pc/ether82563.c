/*
 * Intel 82563, 82566, 82571, 82572, 82573[ev] Gigabit Ethernet Controller
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
enum {
	...
	Itr		= 0x000000c4,	/* Interrupt Throttling Rate */
	...
	Tkabgtxd	= 0x00003004,	/* glci afe band gap transmit ref data, or something */
...
i82563attach(Ether* edev)
{
	...
	ctlr->nrd = Nrd;
	ctlr->ntd = Ntd;
	ctlr->alloc = mallocz(ctlr->nrd*sizeof(Rd)+ctlr->ntd*sizeof(Td) + 255, 1);
	if(ctlr->alloc == nil){
		qunlock(&ctlr->alock);
		return;
	}
...
i82563detach(Ctlr* ctlr)
{
	...
	if((ctlr->type == i82563 || ctlr->type == i82571 || ctlr->type == i82572) && ctlr->rbsz > 8192){
