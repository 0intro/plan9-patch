...
mga4xxscroll(VGAscr *scr, Rectangle r_dst, Rectangle r_src)
{
	...
	assert(Dx(r_src) == Dx(r_dst) && Dy(r_src) == Dy(r_dst));
	...
	if(p_dst.y > p_src.y && p_dst.y < p_src.y + height){
		p_src.y += height - 1;
		p_dst.y += height - 1;
		sgn |= SGN_UP;
	}
	...
	width--;
	start = end = p_src.x + (p_src.y * pitch);

	if(p_dst.x > p_src.x && p_dst.x < p_src.x + width){
		start += width;
		sgn |= SGN_LEFT;
	}
	else
	...
	mgawrite32(mga, FXBNDRY, ((p_dst.x + width) << 16) | p_dst.x);
	mgawrite32(mga, YDSTLEN, (p_dst.y << 16) | height);
