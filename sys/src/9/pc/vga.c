...
vgascreenwin(VGAscr* scr)
{
	...
	if(i = allocmemimage(Rect(0,0,1,1), CMAP8)){
		i->flags |= Frepl;
		i->clipr = scr->gscreen->r;
		memfillcolor(i, 0x444488FF);
...
cornerstring(char *s)
{
	...
	scr = &vgascreen[0];
	if(scr->gscreen == nil || screenputs != vgascreenputs)
		return;
