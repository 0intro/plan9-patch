	...
	if(s == DEVDOTDOT){
		...
		case Q3rd:
			...
			if(cl == nil)
				...
				snprint(up->genbuf, sizeof(up->genbuf), "%d", cl->clientid);
...
drawread(Chan *c, void *a, long n, vlong off)
{
	...
	case Qctl:
		...
		n = snprint(a, n, "%11d %11d %11s %11d %11d %11d %11d %11d %11d %11d %11d %11d ",
	...
	case Qcolormap:
		...
		for(index = 0; index < 256; index++){
			...
			m += snprint((char*)p+m, 4*12*256+1-m, "%11d %11lud %11lud %11lud\n", index, red>>24, green>>24, blue>>24);
		}
...
printmesg(char *fmt, uchar *a, int plsprnt)
{
	...
	*q++ = *a++;
	for(p=fmt; *p; p++){
		switch(*p){
		case 'l':
			pb = snprint(q, l, " %ld", (long)BGLONG(a));
			a += 4;
			break;
		case 'L':
			pb = snprint(q, l, " %.8lux", (ulong)BGLONG(a));
			a += 4;
			break;
		case 'R':
			pb = snprint(q, l, " [%d %d %d %d]", BGLONG(a), BGLONG(a+4), BGLONG(a+8), BGLONG(a+12));
			a += 16;
			break;
		case 'P':
			pb = snprint(q, l, " [%d %d]", BGLONG(a), BGLONG(a+4));
			a += 8;
			break;
		case 'b':
			pb = snprint(q, l, " %d", *a++);
			break;
		case 's':
			pb = snprint(q, l, " %d", BGSHORT(a));
			a += 2;
			break;
		case 'S':
			pb = snprint(q, l, " %.4ux", BGSHORT(a));
			a += 2;
			break;
		}
		l -= pb;
